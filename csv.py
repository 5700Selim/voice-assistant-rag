...
def get_excel_text_chunks(excel_path, text_column="Content"):
    """
    Reads the Excel file and extracts text chunks from a specified column.
    """
    ...
    # Join all paragraphs into one big text string with newlines
...
def main():
    ...
    # Automatically process pre-uploaded Excel only once
    if st.session_state.conversation is None:
        datasheet_path = "datasheet.xlsx"
        if os.path.exists(datasheet_path):
            with st.spinner("Processing datasheet..."):
                text_chunks = get_excel_text_chunks(datasheet_path, text_column="Content")
                if text_chunks:
                    vectorstore = get_vectorstore(text_chunks)
                    st.session_state.conversation = get_conversation_chain(vectorstore)
                    st.success("Datasheet loaded and processed!")
                else:
                    st.error("Failed to process datasheet. Please check the Excel file and column name.")
        else:
            st.warning("Preloaded 'datasheet.xlsx' not found.")
